...
void IoDone(struct UnitData *ud, struct IOSana2Req *req, LONG err, LONG werr)
  {
    ...
    if (!(req->ios2_Req.io_Flags & IOF_QUICK)) ReplyMsg(&req->ios2_Req.io_Message);
    return;
  }
                ...
                if (req = SearchReadRequest(ud, &ud->ud_RxQueue, ud->ud_RxBuffer[6]))
                  {
                    ...
                    for (i = 2; i >= 0; i--) mask &= ud->ud_RxBuffer[i];
                      if (mask == 0xFFFF) req->ios2_Req.io_Flags |= SANA2IOF_BCAST;
...
void BufferOverflow(struct UnitData *ud)
  {
    ...
    if (resent) hw->regs[NE2000_COMMAND] = COMMAND_PAGE0 | COMMAND_START |
...
void SendPacket(struct UnitData *ud, struct IOSana2Req *req)
  {
    ...
    for (cycles = (data_len + 1) >> 1; cycles; cycles--) hw->DMAPort = *ethdata++;
...
void GetHwAddress(struct UnitData *ud)
  {
    ...
    for (i = 0; i < 6; i++) ud->ud_EtherAddress[i] = hw->DMAPort;
