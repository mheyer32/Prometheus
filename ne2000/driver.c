...
#define USE(lib) struct Library * lib = dd->dd_##lib;
#define USE_U(lib) struct Library * lib = ud->ud_##lib;
...
LONG DevOpen(struct IOSana2Req *req reg(a1), LONG unit reg(d0), LONG flags reg(d1), struct DevData *dd reg(a6));
...
const void *FuncTable[] = {DevOpen, DevClose, DevExpunge, DevReserved, DevBeginIO, DevAbortIO, (APTR)-1};

UWORD NSDSupported[] = {
    CMD_READ,     CMD_WRITE,         CMD_FLUSH, S2_DEVICEQUERY, S2_GETSTATIONADDRESS, S2_CONFIGINTERFACE,
    S2_BROADCAST, S2_GETGLOBALSTATS, S2_ONLINE, S2_OFFLINE,     NSCMD_DEVICEQUERY,    0};
...
APTR DevClose(struct IOSana2Req *req reg(a1), struct DevData *dd reg(a6))
{
    ...
    if (req->ios2_BufferManagement)
        FreeMem(req->ios2_BufferManagement, sizeof(struct BuffFunctions));
...
LONG OpenDeviceLibraries(struct DevData *dd)
{
    ...
    if (!(dd->dd_UtilityBase = OpenLibrary("utility.library", 39)))
        return FALSE;
    if (!(dd->dd_PrometheusBase = OpenLibrary("prometheus.library", 2)))
        return FALSE;
...
void CloseDeviceLibraries(struct DevData *dd)
{
    ...
    if (dd->dd_PrometheusBase)
        CloseLibrary(dd->dd_PrometheusBase);
...
LONG PrepareCookie(struct IOSana2Req *req, struct DevData *dd)
{
    ...
    if (req->ios2_BufferManagement) {
        ...
        if (bfun = AllocMem(sizeof(struct BuffFunctions), MEMF_ANY)) {
            bfun->bf_CopyFrom = (APTR)GetTagData(S2_CopyFromBuff, NULL, (struct TagItem *)req->ios2_BufferManagement);
            bfun->bf_CopyTo = (APTR)GetTagData(S2_CopyToBuff, NULL, (struct TagItem *)req->ios2_BufferManagement);
...
LONG RunTask(struct DevData *dd, struct UnitData *ud)
{
    ...
    if (ud->ud_LifeTime = CreateMsgPort()) {
        if (ud->ud_Task = (struct Task *)CreateNewProcTags(NP_Entry, (LONG)UnitTask, NP_Name, (LONG)ud->ud_Name,
                                                           NP_Priority, 6, TAG_END)) {
            WORD i;
...
void S2ConfigInterface(struct UnitData *ud, struct IOSana2Req *req)
{
    ...
    } else if (address_has_all(req->ios2_SrcAddr, 0x00) || address_has_all(req->ios2_SrcAddr, 0xFF)) {
            ...
            while (RingBufferNotEmpty(ud)) {
                ...
                if (req = SearchReadRequest(ud, &ud->ud_RxQueue, ud->ud_RxBuffer[6])) {
                    if (req->ios2_Req.io_Flags & SANA2IOF_RAW)
                        ...
                        offset = 7;
                    CopyB(((struct BuffFunctions *)req->ios2_BufferManagement)->bf_CopyTo, req->ios2_Data,
                          &ud->ud_RxBuffer[offset], len - (offset << 1));
...
volatile struct Ne2000 *FindHardware(struct DevData *dd, WORD unit)
{
    ...
    while (u-- >= 0) {
        board = Prm_FindBoardTags(board, PRM_Vendor, PCI_VENDOR_REALTEK, PRM_Device, PCI_DEVICE_RTL8029, TAG_END);
        if (!board)
            break;
    }

    if (board) {
        Prm_GetBoardAttrsTags(board, PRM_MemoryAddr0, (LONG)&hwbase, TAG_END);
        Prm_SetBoardAttrsTags(board, PRM_BoardOwner, (LONG)dd, TAG_END);
        return hwbase;
    }
    return NULL;
}
...
void GoOnline(struct UnitData *ud)
{
    ...
    hw->regs[NE2000_RX_CONFIG] = RXCFG_BCAST | RXCFG_MCAST | ((ud->ud_Flags & UF_PROMISCUOUS) ? RXCFG_PROM : 0);
    hw->regs[NE2000_INT_STATUS] = 0xFF;
...
void SendPacket(struct UnitData *ud, struct IOSana2Req *req)
{
    ...
    CopyB(((struct BuffFunctions *)req->ios2_BufferManagement)->bf_CopyFrom, datapointer, req->ios2_Data, data_len);
...
struct IOSana2Req *SearchReadRequest(struct UnitData *ud, struct MinList *queue, ULONG type)
{
    ...
    for (req = (struct IOSana2Req *)queue->mlh_Head; req->ios2_Req.io_Message.mn_Node.ln_Succ;
