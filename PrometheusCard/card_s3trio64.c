...
//                bi->Flags |= BIF_VBLANKINTERRUPT;
                ...
                // two sprite images
                const ULONG spriteBuffersSize = MAXSPRITEWIDTH * MAXSPRITEHEIGHT * 4 *2;

                // take sprite image data off the top of the memory (left after DMA)
                bi->MemorySize =
                    (bi->MemorySize - spriteBuffersSize) & ~(1024 - 1);
                ...
                if ((bi->Flags & BIB_BLITTER) != 0) {
                  ...
                  bi->MouseSaveBuffer = bi->MemoryBase + bi->MemorySize;
                  bi->MouseImageBuffer = bi->MemoryBase + bi->MemorySize + spriteBuffersSize /2;
                }
