...
LONG Main(LONG fromWb)
{
    ...
    if (PrometheusBase = OpenLibrary("prometheus.library", 2)) {
        ...
        while (board = Prm_FindBoardTags(board, PRM_Vendor, 0x10EC, PRM_Device, 0x8139, TAG_END)) {
            Printf("$%08lx\n", Prm_ReadConfigLong(board, 0));
            Printf("$%04lx $%04lx\n", Prm_ReadConfigWord(board, 0), Prm_ReadConfigWord(board, 2));
            Printf("$%02lx $%02lx $%02lx $%02lx\n", Prm_ReadConfigByte(board, 0), Prm_ReadConfigByte(board, 1),
                   Prm_ReadConfigByte(board, 2), Prm_ReadConfigByte(board, 3));
            Printf("-------------------------------------------------\n");
        ...
        Printf("prometheus.library V2 not found or initialization failed.\n\n");
    return 0;
}
