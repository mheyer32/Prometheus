...
void StripLeadingSpaces(UBYTE *buffer)
{
    ...
    while (buffer[idx] == 0x09 || buffer[idx] == 0x20)
        idx++;
    if (idx)
        ShiftBufferLeft(buffer, idx);
}
    ...
    if (device != 0xFFFF) {
        if (vendors_file) {
            while (FGets(vendors_file, buffer, bufsize)) {
                if (*buffer == ';')
                    continue; /* comment */
                if (*buffer != 0x09 && *buffer != 0x20)
                    break; /* vendor -> dev not found */
                StripLeadingSpaces(buffer);
...
LONG Main(long fromWB)
{
    ...
    if (PrometheusBase = OpenLibrary("prometheus.library", 2)) {
        ...
        while (board = Prm_FindBoardTags(board, TAG_END)) {
            Prm_GetBoardAttrsTags(board, PRM_Vendor, (ULONG)&vendor, PRM_Device, (ULONG)&device, PRM_Revision,
                                  (ULONG)&revision, PRM_Class, (ULONG)&dclass, PRM_SubClass, (ULONG)&dsubclass,
                                  PRM_SlotNumber, (ULONG)&slot, PRM_FunctionNumber, (ULONG)&function, PRM_BoardOwner,
                                  (ULONG)&owner, TAG_END);
            ...
            Printf("Vendor: \033[1m%s\033[0m\nDevice: \033[1m%s\033[0m\nRevision: %ld.\n", (LONG)vendor_name,
                   (LONG)device_name, revision);
            Printf("Device class %02lx, subclass %02lx.\n", dclass, dsubclass);
            for (blk = 0; blk < 6; blk++) {
                Prm_GetBoardAttrsTags(board, PRM_MemoryAddr0 + blk, (ULONG)&blkaddr, PRM_MemorySize0 + blk,
                                      (ULONG)&blksize, TAG_END);
                if (blkaddr && blksize) {
